...
def build_model(input_shape):
    ...
    conv1 = keras.layers.Conv2D(filters=4, kernel_size=(3, 3), activation='relu',
                                kernel_initializer=keras.initializers.glorot_normal(),
                                bias_initializer=keras.initializers.zeros())(state_input)
    conv2 = keras.layers.Conv2D(filters=8, kernel_size=(3, 3), activation='relu',
                                kernel_initializer=keras.initializers.glorot_normal(),
                                bias_initializer=keras.initializers.zeros())(conv1)
    flatten = keras.layers.Flatten()(conv2)
    advantage_fc = keras.layers.Dense(units=64,
                                      activation='relu',
                                      kernel_initializer=keras.initializers.glorot_normal(),
                                      bias_initializer=keras.initializers.zeros(),
                                      name='advantage_fc')(flatten)
    advantage = keras.layers.Dense(units=len(Policy.ACTIONS),
                                   activation=None,
                                   kernel_initializer=keras.initializers.glorot_normal(),
                                   bias_initializer=keras.initializers.zeros(),
                                   name='advantage')(advantage_fc)

    value_fc = keras.layers.Dense(units=64,
                                  activation='relu',
                                  kernel_initializer=keras.initializers.glorot_normal(),
                                  bias_initializer=keras.initializers.zeros(),
                                  name="value_fc")(flatten)
    ...
    mean = keras.layers.Lambda(lambda x: K.mean(x, axis=1, keepdims=True), name="reduce_mean")(advantage_fc)
    sub = keras.layers.Subtract()([advantage, mean])
    output = keras.layers.Add()([value, sub])
    return keras.Model(inputs=state_input, outputs=output)


class DDQN(bp.Policy):
    ...
    def _preprocess_state(self, state):
        ...
        # we insert it into a CNN add depth
    ...
    def init_run(self):
        ...
        # plot_model(self.model, show_shapes=True, to_file='./dddqn.png')
    ...
    def learn(self, round, prev_state, prev_action, reward, new_state, too_slow):
        ...
        """
        if self.is_double and round % DEFAULT_UPDATE_TARGET_ESTIMATOR_FREQ == 0:
    ...
    def act(self, round, prev_state, prev_action, reward, new_state, too_slow):
        ...
        if round > 0:
            ...
            if DEFAULT_SAVE_NON_POLICY_MEMORIES and not is_policy_action:
                if is_final and self.train_on_terminal_states:
                    self.replay_memory.push(prev_state, prev_action, reward, None)
                else:
                    self.replay_memory.push(prev_state, prev_action, reward, new_state)
